...
class Model():
    ...
    def train(self,X,y):   
        ...
        X_t = pd.DataFrame(columns=features,data=self.scaler.fit_transform(X.loc[:,features]))
    ...
    def eval(self,X):
        ...
        features = X.columns
        X_t = pd.DataFrame(columns=features,data=self.scaler.transform(X[features]))
...
def bootstrap_scores(y_scores,y_true,y_pred,metrics_names,costs=None,priors=None,n_bootstrap=100):
    """
    ...
    all_index_bt = np.empty(0)

    for _ in range(n_bootstrap):
        index_bt = np.random.choice(range(len(y_true)),size=len(y_true),replace=True)
        all_index_bt = np.hstack([all_index_bt,index_bt])
        y_scores_bt = y_scores[index_bt,:]
        #Change inf values to a very large number
        y_scores_bt = np.clip(y_scores_bt,-1e6,1e6)
        
        y_true_bt = y_true[index_bt]
        y_pred = bayes_decisions(scores=y_scores_bt,costs=costs,priors=priors,score_type='log_posteriors')[0]

        for m in metrics_names:
            if m == 'norm_cross_entropy':
                try: 
                    metrics[m] = np.hstack((metrics[m],LogLoss(log_probs=torch.tensor(y_scores_bt),labels=torch.tensor(np.array(y_true_bt)),priors=torch.tensor(priors)).detach().numpy())) if priors is not None else np.hstack((metrics[m],LogLoss(log_probs=torch.tensor(y_scores_bt),labels=torch.tensor(np.array(y_true_bt))).detach().numpy()))
                except:
                    metrics[m] = np.hstack((metrics[m],np.nan))
            elif m == 'norm_expected_cost':
                try:
                    metrics[m] = np.hstack((metrics[m],np.array(average_cost(targets=y_true_bt,decisions=y_pred,costs=costs,priors=priors,adjusted=True))))
                except:
                    metrics[m] = np.hstack((metrics[m],np.nan))
            elif m == 'roc_auc':
                try:
                    metrics[m] = np.hstack((metrics[m],np.array(roc_auc_score(y_true=y_true_bt,y_score=y_scores_bt[:,1]))))
                except:
                    metrics[m] = np.hstack((metrics[m],np.nan))
            else:
                metrics[m] = np.hstack((metrics[m],np.array(eval(f'{m}_score')(y_true=y_true_bt,y_pred=y_pred))))

    return metrics, all_index_bt.astype(int)

def get_metrics(y_scores,y_true,y_pred,metrics_names,cmatrix=None,priors=None):
    """
    ...
    for m in metrics_names:
        if m == 'norm_cross_entropy':
            ...
            metrics[m] = eval(f'{m}_score')(y_true=y_true,y_pred=y_pred)
...
def train_models(model_type,scaler,X_dev,y_dev,random_seeds,hyperp,metrics_names,costs=None,priors=None,CV_type=None,IDs=None,boot_train=False,boot_val=0):
    """
    Trains a model, tunes hyperparameters, calibrates probabilities, and evaluates performance.
    ...
    for random_seed in tqdm(random_seeds):
        CV = CV_type(random_state=random_seed,shuffle=True) if CV_type is not None else StratifiedShuffleSplit(random_state=random_seed)
        ...
        for k,(train_out,val_idx) in enumerate(CV.split(X_dev,y_dev)):
            ...
            logpost_val[f'random_seed_{random_seed}'] = np.vstack((logpost_val[f'random_seed_{random_seed}'],model.eval(X_val)))

            y_true[f'random_seed_{random_seed}'] = np.hstack((y_true[f'random_seed_{random_seed}'],y_dev[val_idx]))

        if boot_val > 0:
            all_metrics[f'random_seed_{random_seed}'], index_bt = bootstrap_scores(y_scores=logpost_val[f'random_seed_{random_seed}'],y_true=y_true[f'random_seed_{random_seed}'],y_pred=y_pred[f'random_seed_{random_seed}'], metrics_names=metrics_names,costs=costs,priors=priors,n_bootstrap=boot_val)
            logpost_val_bt[f'random_seed_{random_seed}'] = logpost_val[f'random_seed_{random_seed}'][index_bt,:]
            y_true_bt[f'random_seed_{random_seed}'] = y_true[f'random_seed_{random_seed}'][index_bt]
            IDs_val_bt[f'random_seed_{random_seed}']= IDs_val[f'random_seed_{random_seed}'].loc[index_bt,'ID']
        else:
            ...
            IDs_val_bt[f'random_seed_{random_seed}']['ID'] = IDs_val['ID'].values
...
def test_model(model,X_test,y_test,metrics_names,costs=None,priors=None,bootstrap=0):
    ...
    if bootstrap > 0:
        all_metrics,index_bt = bootstrap_scores(y_scores=y_scores,y_true=y_test,y_pred=y_pred,metrics_names=metrics_names,costs=costs,priors=priors,n_bootstrap=bootstrap)
        y_scores_bt = model.eval(X_test)[index_bt,:] 
        y_true_bt = y_test[index_bt]    
    else:
        all_metrics = get_metrics(y_scores=y_scores,y_true=y_test,y_pred=y_pred,metrics_names=metrics_names,costs=costs,priors=priors)
        y_scores_bt = y_scores
        y_true_bt = y_test
    
    y_pred_bt = bayes_decisions(scores=y_scores_bt,costs=costs,priors=priors,score_type='log_posteriors')[0]
    
    return all_metrics, y_scores_bt, y_true_bt,y_pred_bt
